...
* TODO Access the camera
  1. [X] file:./bin/set-ip-address.sh
  ...
  4. file:/etc/dhcpd.conf
     ...
     #+END_SRC
  5. [X] systemctl start dhcpd4
  6. [X] Connect camera with ethernet
     #+BEGIN_SRC sh
       ping -b 192.168.1.0
     #+END_SRC
     + Camera is at 192.168.1.XXX
  7. [ ] Add New Monitor
     2. [ ] General/Source Type :: remote
     3. [ ] Source/Remote Host Name :: user:password@192.168.1.1
     4. [ ] Source/Remote Host Path :: videostream.cgi
  8. Warning
     #+BEGIN_SRC conf
