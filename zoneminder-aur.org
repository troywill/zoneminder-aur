...
* Introduction
* Step by step
** BUILDING the zoneminder package
*** TODO Build zoneminder dependencies which are in the AUR
    ...
    4. [ ] Build and install perl-time-parsedate
    5. [ ] Build and install perl-expect
    6. [ ] Build and install perl-net-sftp-foreign
    7. [ ] Build and install perl-php-serialization
    8. [ ] Build and install perl-sys-mmap
    9. [ ] Build and install perl-x10
...
*** TODO Build and install the zoneminder package
    ...
    #+END_SRC
    #+BEGIN_SRC sh
      sudo pacman --upgrade ./zoneminder-1.27.0-4-i686.pkg.tar.xz    
    #+END_SRC

** TODO Configure Apache and PHP
   5. (Optional) Copy config files
    ...
    #+END_SRC
      ...
      #+END_SRC
   7. (Optional, recommended) Copy config files
	...
	cp -i /etc/httpd/conf/httpd.conf httpd.conf.post.zoneminder.install
      #+END_SRC
   8. [ ] Configure /etc/httpd/conf/httpd.conf
   ...
   9. [ ] Configure file://etc/php/php.ini
      #+BEGIN_SRC conf
