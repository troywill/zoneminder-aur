...
* Step by step
  1. [-] Install dependencies
     1. [X] Build and install [[https://aur.archlinux.org/packages/perl-sys-mmap/][perl-sys-mmap/]]
     2. [-] Install php
	1. [X] Read primary documentation https://wiki.archlinux.org/index.php/Apache_HTTP_Server#PHP
        2. [X] Install php php-apache
	   #+BEGIN_SRC sh
             pacman --sync php php-apache
	   #+END_SRC
	3. [X] Make copy of /etc/php/php.ini
	   #+BEGIN_SRC sh
	     cp /etc/php/php.ini php.ini.pre.zoneminder.install
	   #+END_SRC
	4. [X] Make copy of /etc/httpd/conf/httpd.conf
	   #+BEGIN_SRC sh
	     cp /etc/httpd/conf/http.conf httpd.conf.pre.zoneminder.install
	   #+END_SRC
