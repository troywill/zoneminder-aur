...
* Step by step
  ...
  4. [-] Configure /etc/httpd/conf/httpd.conf for PHP
     ...
     2. [X] To use mod_mpm_prefork, open file:/etc/httpd/conf/httpd.conf and replace
	 #+BEGIN_SRC conf
  ...
  5. [X] cd zoneminder && makepkg --syncdeps
     1. [X] pacman --upgrade zoneminder
	...
	2. [X] Make copy of /etc/httpd/conf/httpd.conf
	   ...
	   #+END_SRC
  6. [X] Mysql (see zoneminder.install)
	...
	1. [X] typing (with passsword):
       ...
       ==> "cat /usr/share/zoneminder/db/zm_create.sql | mysql --defaults-file=/etc/mysql/my.cnf"
       ==> "echo 'grant lock tables, alter,select,insert,update,delete on zm.* to 'zmuser'@localhost identified by "zmpass";' | mysql --defaults-file=/etc/mysql/my.cnf mysql"
  7. [X] sudo systemctl start zoneminder
  8. [ ] sudo systemctl enable zoneminder
  9. [ ] PHP in httpd.conf See https://wiki.archlinux.org/index.php/Apache_HTTP_Server
